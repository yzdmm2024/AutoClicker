...
// 兜底路径: IOHID digitizer 事件 -> UITouchesEvent(_setHIDEvent:) -> [UIApplication sendEvent:]
// 注意: iOS16 的 UIEvent 没有 _initWithHIDEvent:, 必须用 UITouchesEvent 类 + _setHIDEvent:
...
// 主路径(iOS16.6.1 实测可用): 进程内合成 UITouch + UITouchesEvent(_allTouches) -> [window sendEvent:]
// iOS16 的 UIEvent 没有 _allTouches ivar, 必须用 UITouchesEvent 类; UITouch 没有 _isTap, 不能设。
static int dispatchViaUITouchesEvent(UIWindow *win, CGFloat x, CGFloat y, BOOL touchDown) {
    if (!win) return 0;
    Class touchClass = NSClassFromString(@"UITouch");
    Class evtClass   = NSClassFromString(@"UITouchesEvent");
    if (!touchClass || !evtClass) return 0;
    ...
    @try {
        ...
        NSLog(@"[AC] UITouchesEvent 派发失败: %@", e.reason);
        return 0;
    }
}
